	...
	*/
	read_map(dmm_text as text, coordX as num, coordY as num, coordZ as num, tag as text, flags as num)
		...
		for(var/posZ = 1 to gridLevels.len)
			...
			if(world.maxy < yMax)
				world.maxy = yMax
				world.log << "[tag] caused map resize (Y) during prefab placement"
			var exampleLine = pick(yLines)
			var xMax = length(exampleLine)/key_len+(coordX-1)
			if(world.maxx < xMax)
				world.maxx = xMax
				world.log << "[tag] caused map resize (X) during prefab placement"
				...
				for(var/internalPosZ = 1 to gridLevels.len)
					...
					for(var/posY = 1 to yLines.len)
						...
						for(var/posX = 1 to length(yLine)/key_len)
							...
							for(var/x in T)
								if(istype(x, /obj) && flags & DMM_OVERWRITE_OBJS)
									del(x)
...
//-- Supplemental Methods ------------------------------------------------------
	...
	proc
		parse_grid(models as text, xcrd, ycrd, zcrd)
			...
			for(var/areaDone = 0 to 1)
				for(var/atomModel in splittext(models, comma_delim))
					...
					if(!atomPath)
						throw EXCEPTION("Attempted to load invalid type [copytext(atomModel, 1, bracketPos)]!")
					var/list/attributes
					if(bracketPos)
						attributes = new()
			...
			// Handle Turfs & Movable Atoms
				...
				if(ispath(atomPath, /turf))
					...
					if(instance) // I hate that we made it so ReplaceWith can return null, it sucks so much
						instance.dir = initial(instance.dir)
					else
						location.dir = initial(instance.dir)
				else
					if (atomPath)
						instance = new atomPath(location)
		...
		loadAttribute(value, list/strings)
			...
			else if(copytext(value, 1, length("list(") + 1) == "list(")
